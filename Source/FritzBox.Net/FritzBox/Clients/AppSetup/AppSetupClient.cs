...
namespace FritzBox.Net.FritzBox.Clients.AppSetup
{
    /// <summary>
    ...
    public class AppSetupClient : FritzTr64Client
    {
        ...
        {
            XDocument document = await InvokeAsync("GetConfig", null);
        ...
        public async Task RegisterAppAsync(AppInfo info)
        {
            ...
            {
                new SoapRequestParameter("NewAppDeviceMAC", info.AppDeviceMAC),
                new SoapRequestParameter("NewAppDisplayName", info.AppDisplayName),
                new SoapRequestParameter("NewAppId", info.AppId),
                new SoapRequestParameter("NewAppInternetRights", info.AppInternetRights ? "1" : "0"),
                new SoapRequestParameter("NewAppPassword", info.AppPassword),
                new SoapRequestParameter("NewAppRight", info.AppRight.ToString()),
                new SoapRequestParameter("NewAppUsername", info.AppUsername),
                new SoapRequestParameter("NewHomeautoRight", info.HomeautoRight.ToString()),
                new SoapRequestParameter("NewNASRight", info.NASRight.ToString()),
                new SoapRequestParameter("NewPhoneRight", info.PhoneRight.ToString())
            };

            await InvokeAsync("RegisterApp", parameters.ToArray());
        }
        ...
        public async Task SetAppVPNASync(AppVpnInfo info)
        {
            ...
            {
                new SoapRequestParameter("NewAppId", info.AppId),
                new SoapRequestParameter("NewIPSecIdentifier", info.IPSecIdentifier),
                new SoapRequestParameter("NewIPSecPreSharedKey", info.IPSecPreSharedKey),
                new SoapRequestParameter("NewIPSecXauthPassword", info.IPSecXauthPassword),
                new SoapRequestParameter("NewIPSecXauthUsername", info.IPSecXauthUsername)

            };
            await InvokeAsync("SetAppVPN", parameters.ToArray());
        }
        ...
        public async Task<string> SetAppMessageReceiverAsync(AppMessageReceiverConfig config)
        {
            List<SoapRequestParameter> parameters = new List<SoapRequestParameter>()
            {
                new SoapRequestParameter("NewAppAVMAddress", config.AppAVMAddress),
                new SoapRequestParameter("NewAppAVMPasswordHash", config.AppAVMPasswordHash),
                new SoapRequestParameter("NewAppId", config.AppId),
                new SoapRequestParameter("NewCryptAlgos", config.CryptAlgos),
                new SoapRequestParameter("NewEncryptionSecret", config.EncryptionSecret)
            };

            XDocument document = await InvokeAsync("SetAppMessageReceiver", parameters.ToArray());
        ...
        {
            await InvokeAsync("ResetEvent", new SoapRequestParameter("NewEventId", eventID));
        }
    }
}
