...
namespace FritzBox.Net.FritzBox.Clients.Host
{
    /// <summary>
    ...
    public class HostsClient : FritzTr64Client
    {
        ...
        {
            XDocument document = await InvokeAsync("GetHostNumberOfEntries", null);
        ...
        {
            XDocument document = await InvokeAsync("GetSpecificHostEntry", new SoapRequestParameter("NewMACAddress", macAddress));
        ...
        {
            XDocument document = await InvokeAsync("GetGenericHostEntry", new SoapRequestParameter("NewIndex", index));
            return new HostEntry()
            {
