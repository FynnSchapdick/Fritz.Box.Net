...
namespace FritzBox.Net.FritzBox.Clients.DeviceConfig
{
    /// <summary>
    ...
    public class DeviceConfigClient : FritzTr64Client
    {
        ...
        {
            XDocument document = await InvokeAsync("FactoryReset", null);
        }
        ...
        {
            XDocument document = await InvokeAsync("Reboot", null);
        }
        ...
        public async Task<string> GetConfigFileAsync(string password)
        {
            ...
            XDocument document = await InvokeAsync("X_AVM-DE_GetConfigFile", new SoapRequestParameter("NewX_AVM-DE_Password", password));
            ...
            Uri uri = default(Uri);
            Uri.TryCreate(ConnectionSettings.BaseUrl, UriKind.Absolute, out uri);
        ...
        public async Task DownloadConfigFileAsync(string password, string path)
        {
            ...
            string configFile = await GetConfigFileAsync(password);
            // get the config file and write it to file system
            byte[] fileContent = await DownloadFileAsync(configFile);
        ...
        {
            XDocument document = await InvokeAsync("X_AVM-DE_SetConfigFile", new SoapRequestParameter("NewX_AVM-DE_Password", password), new SoapRequestParameter("NewX_AVM-DE_ConfigFileUrl", url));
        }
        ...
        {
            XDocument document = await InvokeAsync("GetPersistentData", null);
        ...
        {
            await InvokeAsync("SetPersistentData", new SoapRequestParameter("NewPersistentData", data));
        }
        ...
        {
            XDocument document = await InvokeAsync("X_GenerateUUID", null);
        ...
        {
            await InvokeAsync("ConfigurationStarted", new SoapRequestParameter("NewSessionID", sessionID));
        }
        ...
        {
            XDocument document = await InvokeAsync("ConfigurationFinished", null);
        ...
        {
            XDocument document = await InvokeAsync("X_AVM-DE_CreateUrlSID", null);
